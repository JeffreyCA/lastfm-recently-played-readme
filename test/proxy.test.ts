...
function get(query: string, headers?: Record<string, string>): Request {
  return new Request(`https://lastfm-recently-played.vercel.app/api?${query}`, { headers });
}
...
describe('proxy', () => {
  it('calls the Worker with translated parameters and returns its card', async () => {
    ...
    const res = await handler.fetch(get('user=JeffreyCA01&loved=true&maxage=300'));
    const target = new URL(fetchMock.mock.calls[0]![0] as string);

    expect(target.origin).toBe('https://lastfm-recently-played.jeffreyca.workers.dev');
    expect(target.pathname).toBe('/svg');
    expect(target.searchParams.get('user')).toBe('JeffreyCA01');
    expect(target.searchParams.get('loved')).toBe('between');
  ...
  it('passes a conditional request through and answers 304 with no body', async () => {
    const fetchMock = stubUpstream(new Response(null, { status: 304, headers: { ETag: 'W/"abc"' } }));
